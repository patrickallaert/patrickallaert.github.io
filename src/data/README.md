...
- `events` contains recurring or explicitly dated public activities such as práticas, Guinguettes, initiations, and guest workshops. Guest workshops use `upcoming` or `past` as their status and keep their detail-page content and calendar occurrences after the event so they can remain in the public archive.
